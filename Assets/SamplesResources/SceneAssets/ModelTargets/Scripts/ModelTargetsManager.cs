...
#pragma warning disable 649
    ...
    [SerializeField] ModelTargetBehaviour model360;

    [Header("DataSet Names")]
    [SerializeField] string DataSetStandard = "VuforiaMars_ModelTarget";
    [SerializeField] string DataSetAdvanced = "Vuforia_Motorcycle_Marslander";
    [SerializeField] string DataSet360 = "VuforiaMars_ModelTarget360";
...
#pragma warning restore 649
    ...
    void OnVuforiaStarted()
    {
        ...
        switch (modelTargetMode)
        {
            ...
            case ModelTargetMode.MODE_ADVANCED:
                ...
                break;
            case ModelTargetMode.MODE_360:
                // Start with the 360 Model Target DataSet
                SelectDataSet360(true);
                break;
        }
    }
    ...
    public void SelectDataSet360(bool active)
    {
        if (active)
        {
            ActivateDataSet(DataSet360);
            this.modelTargetMode = ModelTargetMode.MODE_360;
        }
    ...
    void ActivateDataSet(string datasetName)
    {
        ...
        foreach (DataSet ds in dataSets)
        {
            if (ds.Path.Contains(datasetName + ".xml"))
            {
                ...
                else if (datasetName == DataSet360)
                {
                    modelTargetTransform = this.model360.transform;
                }
